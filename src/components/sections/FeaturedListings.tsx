...
export default async function FeaturedListings() {
  const listings=await getPublishedListings(); const featured=listings.filter(l=>l.standing==="prestige").slice(0,3); const items=featured.length?featured:listings.slice(0,3); const {translation, locale}=await getServerTranslation(); const t=translation.home;
  return <section className="py-24 md:py-28"><div className="max-w-6xl mx-auto px-6">
    ...
    <div className="mt-10"><Link href={prefixLocale("/locations", locale)} className="text-sm font-semibold text-navy border-b border-gold pb-1">{t.allProperties}</Link></div>
  </div></section>;
}